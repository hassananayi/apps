...
// APP Name 
app_name = app.attributes[2].value
...
if(app_name == "password"){
  for(i=1;i<max;i++){
    ...
    <div class="input"><input type="password" class="form-control sheet pass"></div>
  </div>`
}
}else{
  for(i=1;i<max;i++){
    ...
    <div class="num">${i}</div>
    <div class="input"><input  type="text" class="form-control sheet name"></div>
    <div class="input"><input type="email" class="form-control sheet email"></div>
  </div>`
}
}
...
for (let i = 0; i < rows.length; i++) {
    ...
    let name_path = rows[i].children[1].children[0].value
    let data_path = rows[i].children[2].children[0].value

    // check if the row is not empty
    if(name_path.length > 0 && data_path.length > 0){
       ...
       if(app_name == "password"){
        ...
        document.getElementById("cards").innerHTML += createBadge(name_path,data_path)
       }
    ...
    //check if ther is no data 
     if(!task > 0){ clear(); return alert("Veuillez entrer vos données.");}
...
function clear(){
  ...
  for(i=0;i< passwords.length;i++){
    ...
    passwords[i].addEventListener("focusin",function(e){
      e.target.attributes[0].value = "text"
    })


    passwords[i].addEventListener("focusout",function(e){
...
function autocomplete(inp, arr) {
  ...
  var names_list = ["BENKOUSSA Hamadi","BOUAKHRI YOUNES","BOUGAA ABDERRAHIM",
  "CHRISTINE Kevin","DE SOUSA Leandro","DIETRICH Kévin","DIALLO Boubacar Sidy",
  "EISENBEIS Chris","FERRANDEZ Daniel","GAUTHIER Ryan","GREBIL Maxime",
  "HEILIG Patrice","HENRIOT Christophe","HENRIOT Eric","HENRIOT Julian","IMBS Sebastien",
  "KUNTZ Sebastien","KUREKCI Gokhan","MASSON Romain","MOR Taner","MURER Stephan",
  "NICEVIC Kenan","REHM Thierry","RISTORTO Vincent","ROSSE Arnaud","SCHAAL Benoit",
  "SCHOETTEL Georges","SOULAIMANA Bounouvamini","STRUB Mike","THOMAE David","TOMEI Suzie","URBAN Matthieu",
  "VESPERINI Gregory","WALTER Alban","WILLER Marc","ABBAS Hassan",
  "ELKADIKI Saleh","ZAGGAY Mohammed","Pierre ","Lucas ","Henri ","Karim ","Jeason ","Houcine ","Julien ",];
  ...
  function createBadge(name,compte){
    ...
    <div class='badge-icon'><img src="./assets/Logo_STEF.gif"></div>
